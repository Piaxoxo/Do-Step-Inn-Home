...
(function () {
  ...
  /* ─────────────────────────────────────────────────────────────────
     The booking engine, three times on one page.

     UP Hotel's embed initialises the element it finds when it loads, and
     a page with three <ibe-up> tags is not what such a script expects —
     in practice only the first one fills. So a spot that stays empty is
     given its own DOCUMENT: an iframe carrying UP Hotel's own snippet,
     nothing else, which makes it the first and only element in there.
     The frame is written with srcdoc, so it stays on this domain — the
     engine sees the same origin as the page, and its height can be read
     back and followed. `<base target="_top">` keeps the booking flow in
     the real window instead of trapping it in the frame.

     Order per spot: the plain element, then its own frame, and only if
     even that draws nothing, the line with the phone number.
     ───────────────────────────────────────────────────────────────── */
  function bookingWidget() {
    var hosts = [].slice.call(document.querySelectorAll("[data-ibe-host]"));
    if (!hosts.length) return;

    function show(host, what) {         /* "widget" | "down" */
      var w = host.querySelector(".ibe"), d = host.querySelector("[data-ibe-down]");
      if (w) w.hidden = what !== "widget";
      if (d) d.hidden = what !== "down";
      host.classList.toggle("is-engine", what === "widget");
    }

    var first = document.querySelector("[data-ibe-host] ibe-up");
    var key = first && first.getAttribute("ibe-key");
    if (!key || !key.trim()) {
      hosts.forEach(function (h) { show(h, "down"); });
      return;
    }
    key = key.trim();

    /* the engine reads `language` when it initialises, so it is set before
       the engine ever loads — a German visitor gets a German mask */
    [].forEach.call(document.querySelectorAll("[data-ibe-host] ibe-up"), function (el) {
      el.setAttribute("language", lang);
    });

    hosts.forEach(function (h) { show(h, "widget"); });
    ...
    /* one document per frame: UP Hotel's snippet and nothing else.
       "</scr" + "ipt>" because this file is inlined into the page. */
    function frameDoc(language) {
      return '<!doctype html><html lang="' + language + '"><head>' +
        ...
        'font-family:Karla,system-ui,-apple-system,sans-serif}</style></head><body>' +
        '<ibe-up ibe-key="' + key + '" language="' + language + '"></ibe-up>' +
        '<scr' + 'ipt src="https://ibe.uphotel.agency/ibe.min.js"></scr' + 'ipt>' +
        '</body></html>';
    }

    function follow(frame) {            /* let the frame set its own height */
      var stop = null;
      function fit() {
        try {
          ...
          if (h > 80) frame.style.height = (h + 8) + "px";
        } catch (e) { if (stop) clearInterval(stop); }
      }
      stop = setInterval(fit, 400);
      setTimeout(function () { clearInterval(stop); }, 40000);
      frame.addEventListener("load", fit);
    }

    function frameIn(host) {
      var box = host.querySelector(".ibe");
      if (!box) return null;
      var frame = document.createElement("iframe");
      frame.className = "ibe__frame";
      frame.title = t("bk.frame") || "Booking";
      frame.setAttribute("scrolling", "no");
      frame.srcdoc = frameDoc(lang);
      box.innerHTML = "";
    ...
    function frameDrew(frame) {
      try {
        var d = frame.contentDocument;
        if (!d || !d.body) return false;
        var el = d.querySelector("ibe-up");
        return drawn(el) || d.body.scrollHeight > 120;
      } catch (e) { return true; }   /* unreadable means it is not ours: leave it */
    }

    hosts.forEach(function (host) {
      var settled = false, frame = null;

      function keep() { settled = true; }

      function step1() {               /* did the plain element fill? */
        if (settled) return;
        if (drawn(host.querySelector("ibe-up"))) { keep(); return; }
        frame = frameIn(host);         /* no: give this spot its own document */
        setTimeout(step2, 5000);
      }

      function step2() {               /* did the frame fill? */
        if (settled || !frame) return;
        if (frameDrew(frame)) { keep(); return; }
        settled = true;
        show(host, "down");            /* the engine is unreachable, not busy */
      }
      ...
      if (window.MutationObserver) {
        ...
        if (box) {
          var mo = new MutationObserver(function () {
            if (drawn(box.querySelector("ibe-up"))) { keep(); mo.disconnect(); }
          });
          mo.observe(box, { childList: true, subtree: true });
        }
      }

      setTimeout(step1, 3500);
      host.addEventListener("befree:relang", function () {
        if (frame) { frame.srcdoc = frameDoc(lang); follow(frame); }
      });
    });
  }
  ...
  function syncWidgetLanguage() {
    [].forEach.call(document.querySelectorAll("[data-ibe-host]"), function (host) {
      ...
      /* a spot running in its own frame reloads that frame */
      if (box.querySelector("iframe")) {
        host.dispatchEvent(new CustomEvent("befree:relang"));
        return;
      }
      /* a plain element can only be re-read by an engine that really is a
         custom element; one that scanned the page once would be handed an
         empty replacement, so that box is left as it stands */
